...
void ABarrelPickup::RollBonuses()
{
	...
	while (BonusRoll <= MaxBonusRoll)
	{
		while (true)
		{
			...
			if (BonusPick == 1)
			{
				BarrelPickupStats.Accuracy += FMath::RandRange(0.02f, 0.035f);
				BonusRoll += 1;
				break;
			}
			if (BonusPick == 2)
			{
				BarrelPickupStats.FireRate += FMath::RandRange(0.05f, 0.08f);
				BonusRoll += 1;
				break;
			...
			if (BonusPick == 3)
			{
				BarrelPickupStats.BaseDamage += FMath::RandRange(15.0f, 30.0f);
				BonusRoll += 1;
				break;
			}
			if (BonusPick == 4)
			{
				BarrelPickupStats.MagazineSize += FMath::RandRange(2, 4);
				BonusRoll += 1;
				break;
			}
			if (BonusPick == 5)
